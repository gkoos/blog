...
This guide gives you a repeatable process for evaluating an npm package before you add it. It takes 5–10 minutes. It won't guarantee safety - nothing will - but it will help you make an informed decision rather than an optimistic one.
...
On the **npm page**, go to the Versions tab. Is there a recognisable release cadence - monthly, quarterly, whatever - or a 2-year gap followed by a burst of activity? Long gaps followed by sudden updates are sometimes a red flag in themselves: accounts do get taken over.
...
1. **Does it have provenance?** This is the single clearest signal that the published package is what the author intended to publish. No other check gives you that.
2. **Does it have unexplained install scripts?** Code runs on your machine the moment you run `npm install`. Everything else in this guide assumes you get to review the code first.
3. **Is the maintainer responsive?** Go to the Issues tab and look at the oldest open ones. Unacknowledged issues from 18 months ago tell you what support looks like when something goes wrong.
